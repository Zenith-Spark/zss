          ...
          {notificationsData.length > 0 ? (
            notificationsData.map((notif) => (
              <Link 
                ...
                key={notif.id}
                className={`w-full py-2 px-3 mb-2 cursor-pointer hover:border-b transition-colors duration-200 flex items-start justify-between ${notif.is_read ? 'text-gray-500' : 'font-medium'}`}
              >

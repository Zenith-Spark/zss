...
export const ButtonOne = ({ buttonValue, iconValue, IconButton, Clicked, disabled }) => (
  <button
    ...
    className={`flex items-center justify-center text-sm transition-all duration-300 cursor-pointer shadow-md active:translate-y-1 ${
      IconButton
        ? 'w-9 h-9 rounded-full bg-yellow-600 text-white border border-yellow-600'
        : 'py-2 px-4 rounded-full bg-yellow-600 text-white border border-yellow-600'
    } hover:bg-transparent hover:text-yellow-600 ${
      disabled ? 'opacity-50 cursor-not-allowed hover:bg-yellow-600 hover:text-white' : ''
    }`}
  >
...
export const ButtonTwo = ({ buttonValue, iconValue, IconButton, Clicked, disabled }) => (
  <button
    ...
    className={`flex items-center justify-center text-sm transition-all duration-300 cursor-pointer shadow-md active:translate-y-1 ${
      IconButton
        ? 'w-9 h-9 rounded-full border border-yellow-600 text-yellow-600 bg-transparent'
        : 'py-2 px-4 rounded-full border border-yellow-600 text-yellow-600 bg-transparent'
    } hover:bg-yellow-600 hover:text-white ${
      disabled ? 'opacity-50 cursor-not-allowed hover:bg-transparent hover:text-yellow-600' : ''
    }`}
  >
    {buttonValue && <span>{buttonValue}</span>}
    {iconValue && <span className="ml-2">{iconValue}</span>}
